...
function initialized() {
    ...
    s = $('#LoadingTable').DataTable({
        ajax: {
            url: '../Loading/GetLoadingList',
            type: "POST",
            datatype: "json"
        },
        autoWidth: false,
...
function initialized_outboundItem() {
        ...
        columns: [
            ...
            { title: "Plan Qty", data: "SUMQty" },
            {
                title: "Actual Qty", data: function (x) {
                    ...
                    return data = "<input type='number' class='form-control clear' id='ActualQty" + x.PickID + "' value='" + x.ActualQty + "'>"
                }
            },
            {
                title: "Action",
                render: function (data, type, full, meta) {
                    data = '<input type="hidden" id="withChange_' + meta.row + '" value="0" class="withChange_' + meta.row + '">'
                        + '<button type="button" class="btn btn-success btn-sm btn-flat btnedit_' + meta.row + ' btnedit" data-count="' + meta.row + '" data-toggle="tooltip" data-placement="top" title="Save">'
    ...
    $('#OutboundItemTable_view tbody').on('click', '.btnedit', function () {
        var count = $(this).data('count');
        var tabledata = $('#OutboundItemTable_view').DataTable();
        var data = tabledata.row($(this).parents('tr')).data();
